...
public class GenerarPDFListas {
	...
	public GenerarPDFListas(String ruta, int renglones, String dataPDF[][]) {
		try {
			...
			tab.setWidths(new float[] {2, 2, 2, 7, 1});
			PdfPCell cellProd = new PdfPCell(getHeader("Pedido")),
					 cellCant = new PdfPCell(getHeader("Fecha")),
					 cellCost = new PdfPCell(getHeader("Cliente")),
					 cellTota = new PdfPCell(getHeader("Nombre de cliente")),
					 cellGana = new PdfPCell(getHeader("Total"));
			...
			for(int i = 0; i < renglones; i++) {
				...
				total = total + (Double.parseDouble(dataPDF[i][4].substring(2, dataPDF[i][4].length())));
			}
			doc.add(tab);
			PdfPTable tabResults = new PdfPTable(5);
			tabResults.setWidths(new float[] {2, 2, 2, 7, 1});
			...
			tabResults.addCell(getHeader(renglones + " productos."));
